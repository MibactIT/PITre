...
namespace DocsPaDB.Query_DocsPAWS
{
    ...
    public class Utenti : DBProvider
    {
        ...
        public bool InsertCanalePrefMail(string idCorrGlobale)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    dbProvider.ExecuteScalar(out idCanaleMail, queryString);
        ...
        public bool CheckExistDettagliCorr(string idCorr)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    using (System.Data.IDataReader dr = dbProvider.ExecuteReader(sql))
                    {
                        if (dr == null)
                            throw new Exception();
        ...
        public bool InsertDettagli(string systemId, DocsPaVO.addressbook.DettagliCorrispondente dettagli)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    logger.Debug(sql);
                    //database.executeNonQuery(insertDettagliQuery);
                    result = dbProvider.ExecuteNonQuery(sql);
                }
            }
            catch (Exception)
            {
        ...
        public void IsCodRubricaPresente(string codRubrica, string tipoCorr, string idAmm, string idReg, bool inRubricaComune, out DataSet dataSet)
        {
            ...
            //#region commentato perchè nel verificarsi di ciascuna condizione quello che fa è aggiungere sempre AND A.DTA_FINE IS NULL
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                logger.Debug(sql);
                //database.fillTable(codiceString,dataSet,"CODICE");
                dbProvider.ExecuteQuery(out dataSet, "CODICE", sql);
                if (dataSet.Tables["CODICE"].Rows.Count > 0)
                {
                    if (inRubricaComune)
                    {
        ...
        public void GetCanali(out DataSet dataSet)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_DocumentTypes3");
                string sql = q.getSQL();
                dbProvider.ExecuteQuery(out dataSet, "CANALI", sql);
            }
        ...
        public void GetRuoliSuperiori(out DataSet dataSet, DocsPaVO.utente.Ruolo ruolo)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                //database.fillTable(queryString,dataSet,"RUOLI_SUP");
                dbProvider.ExecuteQuery(out dataSet, "RUOLI_SUP", sql);
            }
        }
        ...
        public void InsertRUOEUtente(string sysId, DocsPaVO.utente.Corrispondente parent)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string res;
                dbProvider.InsertLocked(out res, sql, "DPA_RUOE_UTENTE");
            }
        ...
        public string InsertUtente(DocsPaVO.utente.Corrispondente corr)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string res;
                dbProvider.InsertLocked(out res, sql, "DPA_CORR_GLOBALI");
                return res;
            }
        ...
        public bool InsertOccasionale(DocsPaVO.utente.DatiModificaCorr datiModifica, out string newIdCorrGlobali, out string message)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                sp_params.Add(res);
                //this.BeginTransaction();
                int resultStore = dbProvider.ExecuteStoredProcedure("INS_OCC", sp_params, null);
        ...
        public string GetSignTypePreference(string idPeople)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                try
                {
                    if (dbProvider.ExecuteQuery(out ds, "signpreference", q.getSQL()))
                    {
                        if (ds.Tables["signpreference"].Rows.Count > 0)
                        {
                            DataRow dr = ds.Tables["signpreference"].Rows[0];
        ...
        public DocsPaVO.utente.Corrispondente GetCorrispondenteByCodRubricaNotDisabled(string id_amm, string cod_rubrica, DocsPaVO.addressbook.TipoUtente tipoIE)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                try
                {
                    if (dbProvider.ExecuteQuery(out ds, "corrispondenti", q.getSQL()))
                    {
                        DocsPaVO.utente.Corrispondente corrispondente = new DocsPaVO.utente.Corrispondente();
                        if (ds.Tables["corrispondenti"].Rows.Count >= 1)
                        {
        ...
        public void DettagliCorrispondenti(out DataSet dataSet, DocsPaVO.addressbook.QueryCorrispondente objQueryCorrispondente)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                //database.fillTable(commandString1,dataSet,"DETTAGLI");
                dbProvider.ExecuteQuery(out dataSet, "DETTAGLI", sql);
            }
        }
        ...
        private string GetQueryChildrenGruppo(string codiceRubrica)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    //database.fillTable(parentString, dataSet, "PARENT");
                    dbProvider.ExecuteQuery(out dataSet, "PARENT", sql);
                    if (dataSet.Tables["PARENT"].Rows.Count == 0)
                    {
                        return sql;
                    }
        ...
        public DataSet GetRuoliUtenteInterno(string codiceRubrica)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    dbProvider.ExecuteQuery(dataSet, "RUOLI", sql);
                }
            }
            catch (Exception ex)
            {
        ...
        public ArrayList GetCorrGruppi(DocsPaVO.addressbook.QueryCorrispondente qco)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    //database.fillTable(queryString,dataSet,"CORR_GRUPPI");
                    dbProvider.ExecuteQuery(out dataSet, "CORR_GRUPPI", queryString);
                    if (objQuery.codiceRubrica != null && objQuery.getChildren)
                    {
                        //nel caso in cui si ottengono gli appartenenti al gruppo, che sono utente o UO
                        foreach (DataRow dr in dataSet.Tables["CORR_GRUPPI"].Rows)
        ...
        public ArrayList GetListaUtenti()
        {
            ...
            //DocsPaUtils.Query qMinId = DocsPaUtils.InitQuery.getInstance().getQuery("S_UTENTI_MIN_SYSTEM_ID_GROUP_BY_USER_ID");
            //string sqlMinId = qMinId.getSQL();
            //logger.Debug(sqlMinId);

            //ArrayList idList = new ArrayList();

            //string idString = String.Empty;

            //DataSet dataSetMinId;
            //using (DBProvider dbProvider = new DBProvider())
            //{

            //    dbProvider.ExecuteQuery(out dataSetMinId, sqlMinId);
            //    if (dataSetMinId != null)
            //    {
            //        idString = "(";
            //        foreach (DataRow row in dataSetMinId.Tables[0].Rows)
            //        {
            //            idString = idString.ToString() + row[1] + ",";
            //        }

            //        idString = idString.Remove(idString.Length - 1) + ")";
            //    }
            //    logger.Debug("idList = " + idString);

            //}

            ArrayList utenti = new ArrayList();

            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_UTENTI_BY_ID_LIST");
            //q.setParam("sysIdList", idString);
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteQuery(out dataSet, sql);
                if (dataSet != null)
                {
                    foreach (DataRow row in dataSet.Tables[0].Rows)
                    {
                        ...
                        ut.descrizione = row[2].ToString();
                        ut.nome = row[3].ToString();
                        ut.cognome = row[4].ToString();
                        //ut.idAmministrazione = row[5].ToString();
                        //ut.codiceAmm = row[6].ToString();
                        utenti.Add(ut);
                    }
                }
            }
        ...
        public void GetRootUO(out DataSet dataSet, DocsPaVO.addressbook.QueryCorrispondente objQueryCorrispondente)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                //database.fillTable(queryUO,ds,"UO");
                dbProvider.ExecuteQuery(out dataSet, "UO", sql);
            }
        }
        ...
        public void RicercaUOParentInt(DataSet dataSet, DataRow corrRow)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                logger.Debug(sql);
                //database.fillTable(commandString2,dataSet,"UO");
                dbProvider.ExecuteQuery(dataSet, "UO", sql);
            }
        }
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (objQueryCorrispondente.codiceRubrica != null)
                    {
                        ...
                        //database.fillTable(parentString,dataSet,"PARENT");
                        dbProvider.ExecuteQuery(out dataSet, "PARENT", parentString);
                    ...
                    //database.fillTable(commandString1,dataSet,"CORRISPONDENTI");
                    dbProvider.ExecuteQuery(out dataSet, "CORRISPONDENTI", commandString1);
                    ArrayList list = new ArrayList();
                    //logger.Debug(commandString1);
                    //ArrayList ruoli=new ArrayList();
                    foreach (DataRow corrRow in dataSet.Tables["CORRISPONDENTI"].Rows)
        ...
        public ArrayList ListaCorrispondentiIntInterop(DocsPaVO.addressbook.QueryCorrispondente qco)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                if (objQueryCorrispondente.isRuoloDefined())
                {
                    ...
                    commandString = q.getSQL();
                    string inString = "";
                    dbProvider.ExecuteQuery(out dataSet, "ID_UO", commandString);
                    foreach (DataRow rowIdUo in dataSet.Tables["ID_UO"].Rows)
                    {
                        if (!inString.Equals(string.Empty))
                            inString += ", ";
                    ...
                    commandString = q.getSQL();
                    logger.Debug("commandString_ruolo=" + commandString);
                    dbProvider.ExecuteQuery(out dataSet, "CORRISPONDENTI", commandString);
                    ArrayList list = new ArrayList();
                    foreach (DataRow corrRow in dataSet.Tables["CORRISPONDENTI"].Rows)
                    {
                        RicercaUOParentInt(dataSet, corrRow);
        ...
        public DocsPaVO.utente.Corrispondente GetCorrispondenteInt(DocsPaVO.addressbook.QueryCorrispondente qco, bool notDisabled)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (!dbProvider.ExecuteQuery(out dataSet, command)) throw new Exception();
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (objQueryCorrispondente.codiceRubrica != null)
                    {
                        ...
                        string parentString = q.getSQL() + whereCond;
                        logger.Debug("lista corrispongenti occ: " + parentString);
                        dbProvider.ExecuteQuery(dataSet, "PARENT", parentString);
        ...
        public ArrayList ListaCorrispondentiEst(DocsPaVO.addressbook.QueryCorrispondente qco)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    //database.fillTable(commandString1,dataSet,"CORRISPONDENTI");
                    dbProvider.ExecuteQuery(dataSet, "CORRISPONDENTI", commandString1);
                    ArrayList list = new ArrayList();

                    //riempimento dell'oggetto finale
                    foreach (DataRow corrRow in dataSet.Tables["CORRISPONDENTI"].Rows)
                    {
                        #region ricerca delle UO parent: viene riempita una tabella ottimizzata - COMMENTATO
                        //ricerca delle UO parent: viene riempita una tabella ottimizzata
                        /*string commandString2="";
                        if(corrRow["CHA_TIPO_URP"].ToString().Equals("U"))
                        {
        ...
        public ArrayList ListaUOAut(DocsPaVO.addressbook.QueryCorrispondente qco, ArrayList listaUO)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    logger.Debug(commandString1);
                    dbProvider.ExecuteQuery(out dataSet, "UO_AUT", commandString1);
                    foreach (DataRow corrRow in dataSet.Tables["UO_AUT"].Rows)
                    {
                        DocsPaVO.utente.UnitaOrganizzativa corrispondenteUO = new DocsPaVO.utente.UnitaOrganizzativa();
                        corrispondenteUO.systemId = corrRow["SYSTEM_ID"].ToString();
        ...
        public void GetListaDiritti(out DataSet dataSet, string idObj)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                logger.Debug(sql);
                //db.fillTable(queryStringRuoli,dataSet,"DIRITTI_RUOLI");
                dbProvider.ExecuteQuery(out dataSet, "DIRITTI_RUOLI", sql);
            }
        ...
        public void GetListaRuoli(out DataSet dataSet, string idObj, string IDAMM)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                logger.Debug(sql);
                //db.fillTable(queryStringUtenti,dataSet,"DIRITTI_UTENTI");
                dbProvider.ExecuteQuery(out dataSet, "DIRITTI_UTENTI", sql);
            }
        ...
        public void GetListaDiritti_Deleted(out DataSet dataSet, string idObj)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteQuery(out dataSet, "DIRITTI_RUOLI_DELETED", sql);
            }
        ...
        public void GetListaRuoli_Deleted(out DataSet dataSet, string idObj, string IDAMM)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteQuery(out dataSet, "DIRITTI_UTENTI_DELETED", sql);
            }
        ...
        public void GetListaStoriaDiritti(out DataSet dataSet, string idObj, string tipoObj)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteQuery(out dataSet, "STORIA_DIRITTI", sql);
            }
        ...
        public void GetLibrary(out string library)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteScalar(out library, sql);
                }
        ...
        public void GetDominio(out string dominio, string idPeople)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (!dbProvider.ExecuteScalar(out dominio, sql))
                        throw new System.Exception();
                }
            }
            catch (Exception ex)
        ...
        public bool UserLogin(out string peopleId, string userName, string password, string idAmm, string modulo)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (!dbProvider.ExecuteScalar(out peopleId, command))
                        throw new System.Exception();
                ...
                for (int i = 0; i < registri.Count; i++)
                {
                    ...
                    if (reg.stato.Equals("C"))
                    {
                        if (reg.dataUltimoProtocollo != null && reg.dataUltimoProtocollo != "" && reg.dataUltimoProtocollo.Substring(6, 4).Equals(DocsPaUtils.Functions.Functions.GetDate(false).Substring(6, 4)))
                        {
                            ...
                            reg.ultimoNumeroProtocollo = db.executeScalar(queryString).ToString();*/
                            string res;
                            dbProvider.ExecuteScalar(out res, GetUltimoNumProto(reg.systemId));
        ...
        public string GetCodiceRegistroBySystemId(string idRegistro)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                dbProvider.ExecuteScalar(out codiceRegistro, sql);
                return codiceRegistro;
            }
        }
        ...
        public string GetEmailUtente(string idPeople)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteScalar(out emailMittente, sql);
        ...
        public string GetFromEmailUtente(string idPeople)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteScalar(out emailFromMittente, sql);
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (registro.stato.Equals("C"))
                    {
                        ...
                        string sqlQuery = q.getSQL();
                        string res;
                        dbProvider.ExecuteScalar(out res, sqlQuery);
                        registro.ultimoNumeroProtocollo = res;
                        AggiornaStorico(idPeople, idCorrGlobali, registro);
                    }
                }
        ...
        public string GetDST(string userId)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    using (System.Data.IDataReader dr = dbProvider.ExecuteReader(queryString))
                    {
        ...
        public DocsPaVO.utente.Corrispondente GetCorrispondente(string idCorrispondente, bool isEnable)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteScalar(out idPeople, queryString);
                }
            }
            catch (Exception) { }
        ...
        public DocsPaVO.utente.Corrispondente GetCorrispondenteByCodRubrica(string id_amm, string cod_rubrica, DocsPaVO.addressbook.TipoUtente tipoIE, string condRegistri, bool storicizzato)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (dbProvider.ExecuteQuery(out ds, "corrispondenti", commandText))
                    {
                        DocsPaVO.utente.Corrispondente corrispondente = new DocsPaVO.utente.Corrispondente();
                        if (ds.Tables["corrispondenti"].Rows.Count >= 1)
                        {
        ...
        public DocsPaVO.utente.Corrispondente GetCorrispondenteBySystemID(string systemID)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(systemID))
                {
                    ...
                    {
                        int num;
                        if (Int32.TryParse(systemID, out num))
                        {
        ...
        public void GetIdAmmUtente(out ArrayList List_idAmm, string userName)
        {
            ...
            */
            try
            {
                ...
                //    while (reader.Read())
                //    {
                //        if (reader["ID_AMM"] != DBNull.Value)
                //            List_idAmm.Add(reader["ID_AMM"].ToString());
                //    }
                //}
        ...
        public String[] GetMementoUtente(string idPeople, string IdAmministrazione)
        {
            ...
            {
                DataSet dataSet;
                dbProvider.ExecuteQuery(out dataSet, "PEOPLE", sql);
                String retval = "";
        ...
        public DocsPaVO.utente.Utente GetUtenteByEmail(string idAmm, string email)
        {
            ...
            if (ut != null && ut.idPeople != null)
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    logger.Debug(q_1.getSQL());
                    DataSet dataSet;
                    dbProvider.ExecuteQuery(out dataSet, "IDCORRGLOBALI", q_1.getSQL());
            ...
            */
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                using (IDataReader reader = dbProvider.ExecuteReader(sql))
                {
                    ...
                    {
                        utente = new DocsPaVO.utente.Utente();
                        utente.idPeople = reader["SYSTEM_ID"].ToString();
                        utente.userId = reader["USER_ID"].ToString();
                        utente.descrizione = reader["VAR_COGNOME"].ToString() + " " + reader["VAR_NOME"].ToString();
                        utente.telefono = reader["VAR_TELEFONO"].ToString();
                        utente.email = reader["EMAIL_ADDRESS"].ToString();
                        utente.notifica = reader["CHA_NOTIFICA"].ToString();
                        utente.amministratore = FromCharToBool(reader["CHA_AMMINISTRATORE"].ToString());
                        utente.assegnante = FromCharToBool(reader["CHA_AMMINISTRATORE"].ToString());
                        utente.assegnatario = FromCharToBool(reader["CHA_AMMINISTRATORE"].ToString());
                        utente.idAmministrazione = reader["ID_AMM"].ToString();
                        utente.notificaConAllegato = FromCharToBool(reader["CHA_NOTIFICA_CON_ALLEGATO"].ToString());
                        utente.sede = reader["VAR_SEDE"].ToString();
                        utente.matricola = (reader["MATRICOLA"] != DBNull.Value ? reader["MATRICOLA"].ToString() : null);
                        utente.tipoCorrispondente = "P";
                        //Abbatangeli Gianlugigi - carico le applicazioni relative all'utente
                        //06/03/2017: commentata la riga sotto
                        //utente.extApplications = this.getApplicazioniUtente(utente.idPeople);
                        utente.cognome = reader["VAR_COGNOME"].ToString();
                        utente.nome = reader["VAR_NOME"].ToString();

                        if (reader.GetSchemaTable().Select("ColumnName='IDCORRGLOBALI'").Length > 0)
                        {
                            utente.systemId = reader["IDCORRGLOBALI"].ToString();
                        }
                    }
                }
            }
        ...
        public void GetRuoliUOUtenteForEnabledActions(DataSet dataSet, string idPeople, string codicetipofunzione, string IdAmm)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteQuery(dataSet, "RUOLIXFUNC", sql);
            }
        }
        //*************************************************************************************
        //*************************************************************************************
        ...
        public ArrayList GetFunzioni(string idRuolo)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                q.setParam("param1", "C.ID_RUOLO_IN_UO=" + idRuolo);
                string sql = q.getSQL();
                dbProvider.ExecuteQuery(out dataSet, "FUNZIONI", sql);
                ArrayList funzioni = new ArrayList();
                foreach (DataRow funzioneRow in dataSet.Tables["FUNZIONI"].Rows)
                {
                    DocsPaVO.utente.Funzione funzione = new DocsPaVO.utente.Funzione(
        ...
        public ArrayList VisibilitaRuolo(string docNumber, string idGruppi)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                string sql = q.getSQL();
                logger.Debug(sql);
                dbProvider.ExecuteQuery(out dataSet, "VISIB_RUOLI", sql);
                ArrayList visRuolo = new ArrayList();
                foreach (DataRow visRuoloRow in dataSet.Tables["VISIB_RUOLI"].Rows)
                {
                    visRuolo.Add(visRuoloRow["PERSONORGROUP"].ToString());
        ...
        public ArrayList getListaStoriciMittenti(string idProfile, string tipo)
        {
            ...
            try
            {
                ...
                {
                    DataSet dataSet = new DataSet();
                    dbProvider.ExecuteQuery(out dataSet, "STORICO", sql);
        ...
        public bool CheckLdapLogin(string userId)
        {
            //bool result = false;
            bool result = true;
            try
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("CHECK_LDAP_LOGIN");
                ...
                if (dr.Read())
                {
                    ...
                    //    //{
                    //    //    result = (Convert.ToDecimal(dr[0]) < 30);
                    //    //}
                    result = true;
                }
                ...
                q = DocsPaUtils.InitQuery.getInstance().getQuery("DELETE_LDAP_LOGIN");
                q.setParam("userId", userId);
                sql = q.getSQL();
        ...
        public string RemoveVisibilita(string docNumber, DocsPaVO.utente.Corrispondente corr)
        {
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                int rows = 0;
                this.BeginTransaction();
                rtn = dbProvider.ExecuteNonQuery(sql, out rows);
                if (!(rtn && rows > 0))
                {
        ...
        public bool AddUserFilenet(string userID, string userGroup, string archivio, string userFullName)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    this.BeginTransaction();
                    dbProvider.ExecuteNonQuery(sql, out rows);
                    if (rows == 0)
        ...
        public bool DeleteUserFilenet(string username)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    dbProvider.ExecuteNonQuery(sql2, out rows);
                    if (rows == 0)
                        this.RollbackTransaction();
                    else
                        this.CommitTransaction();
        ...
        public string GetPasswordUserFilenet(string username)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteScalar(out user_password, sql);
        ...
        public bool DeleteCorrispondenteEsterno(string idCorrglobali, int flagListe, DocsPaVO.utente.InfoUtente user, out string message)
        {
            ...
            try
            {
                ...
                if (idCorrglobali != null && idCorrglobali != "")
                {
                    ...
                    using (DBProvider dbProvider = new DBProvider())
                    {
                        retProc = dbProvider.ExecuteStoreProcedure("SP_DELETE_CORR_ESTERNO", parameters);
                ...
                if (datiModifica != null)
                {
                    ...
                    */
                    ...
                    using (DBProvider dbProvider = new DBProvider())
                    {
                        ...
                        retProc = dbProvider.ExecuteStoreProcedure("SP_MODIFY_CORR_ESTERNO_IS", parameters);
                        /*
                            VALORI RITORNATI
        ...
        public string GetRegistroDaCodice(string codice, string idAmm)
        {
            ...
            try
            {
                ...
                queryMng.setParam("param2", "VAR_CODICE = '" + codice + "' AND ID_AMM = " + idAmm);

                logger.Info(queryMng.getSQL());
        ...
        public string GetFromCorrGlobGeneric(string campo, string condizione)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteScalar(out retValue, sql);
            }
            return retValue;
        }
        ...
        public void invalidaCorr(string dataFine, string corrSystemId, string cod_rubrica)
        {
            ...
            int rowsAffected;
            using (DBProvider dbProvider = new DBProvider())
                dbProvider.ExecuteNonQuery(insertString, out rowsAffected);
        ...
        public int resetCorrVarInsertIterop(string corrSystemId, string val)
        {
            ...
            int rowsAffected;
            using (DBProvider dbProvider = new DBProvider())
                dbProvider.ExecuteNonQuery(resetString, out rowsAffected);
        ...
        public void GetCorrByEmail(string email, string idRegistri, string idAmm, out DataSet ds)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteQuery(out ds, "CorrispondentiByMail", sql);
            }
        ...
        public void GetCorrByEmailAndDescr(string email, string descr, string idRegistri, string idAmm, out DataSet ds)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteQuery(out ds, "CorrispondentiByMailAndDescr", sql);
            }
        ...
        public bool GetNumCorrInAMM(string codiceCorr, string idAmm, string codiceAmm)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteScalar(out countCorr, sql);
        ...
        public List<DocsPaVO.Qualifica.Qualifica> GetQualifiche(int id_amm)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteQuery(out dataSet, "QUALIFICHE", sql);
        ...
        public bool CheckUniqueCodiceQualifica(string codiceQualifica, string idAmministrazione)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteScalar(out retParam, sql);
        ...
        public List<DocsPaVO.Qualifica.PeopleGroupsQualifiche> GetPeopleGroupsQualifiche(String idAmm, String idUo, String idGruppo, String idPeople)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteQuery(out dataSet, "PEOPLEGROUPS_QUALIFICHE", sql);
        ...
        public List<DocsPaVO.Qualifica.PeopleGroupsQualifiche> GetPeopleGroupsQualificheByIdPeople(String idPeople)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteQuery(out dataSet, "PEOPLEGROUPS_QUALIFICHE_BY_IDPEOPLE", sql);
        ...
        public List<DocsPaVO.utente.Utente> GetUtentiByCodQualifica(String codQualifica)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteQuery(out dataSet, "UTENTI", sql);
        ...
        public List<DocsPaVO.utente.Corrispondente> GetPrefChannelAllDest(string idProfile, string typeDest)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(idProfile) && (!string.IsNullOrEmpty(typeDest)))
                {
                    ...
                    using (DBProvider dbProvider = new DBProvider())
                    {
                        dbProvider.ExecuteQuery(ds, q.getSQL());
                    }
        ...
        public DocsPaVO.addressbook.CorrespondentDetails getCorrespondentDetails(string idCorr)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    dbProvider.ExecuteQuery(out dataSet, "CORRESPONDENTS", sql);
        ...
        public bool AddBrowserInfo(BrowserInfo browser, string idPeople, string userName, string ipAddress)
        {
            ...
            try
            {
                ...
                if (insert)
                {
                    ...
                    using (DBProvider dbProvider = new DBProvider())
                    {
                        dbProvider.ExecuteNonQuery(query, out res);
                        if (res > 0)
                            result = true;
                        else
                            result = false;
        ...
        public List<string> GetIdOpportunityList(string cod_people)
        {
            ...
            try
            {
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (dbProvider.ExecuteQuery(out ds, "opportunity", q.getSQL()))
                    {
                        DocsPaVO.utente.Corrispondente corrispondente = new DocsPaVO.utente.Corrispondente();
                        if (ds != null && ds.Tables[0].Rows.Count > 0)
                        {
        ...
        private bool isArchivistaDeposito(string idGruppo)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteScalar(out ret, selectString);
        ...
        public bool InsertHSMPin(HSMPin hsmPin)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    execute = dbProvider.InsertLocked(out res, sql, "DPA_DPA_HSMPIN");
        ...
        public bool UpdateHSMPin(HSMPin hsmPin)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    execute = dbProvider.ExecuteNonQuery(q.getSQL());
        ...
        public bool InsertHSMParameters(HSMParameters hsmParameters)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    execute = dbProvider.InsertLocked(out res, sql, "DPA_HSMPARAMETERS");
        ...
        public List<string> GetListaCapComuni(string prefixCap, string comune)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (dbProvider.ExecuteQuery(out ds, "cap", query))
                    {
        ...
        public InfoComune GetProvinciaComune(string comune)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (dbProvider.ExecuteQuery(out ds, "provincia", query))
                    {
        ...
        public List<string> GetListaComuni(string prefixComune)
        {
            ...
            try
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    if (dbProvider.ExecuteQuery(out ds, "comune", query))
                    {
        ...
        public List<Corrispondente> GetListaUoProcedimenti(string searchString)
        {
            List<Corrispondente> list = null;
            logger.Debug("BEGIN");

            try
            {
                DocsPaUtils.Query query = DocsPaUtils.InitQuery.getInstance().getQuery("S_GET_LISTA_UO_PROCEDIMENTI");
                query.setParam("search_string", searchString);
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (ds != null && ds.Tables[0] != null)
                    {
                        ...
                        foreach (DataRow row in ds.Tables[0].Rows)
                        {
                            ...
                            corr.codiceAmm = row["VAR_CODICE_AMM"].ToString();
            ...
            {
                logger.Debug("Errore in GetListaUoProcedimenti - " + ex.Message);
                list = null;
            }
        ...
        /* REPERIMENTO LISTA AOO PER UN UTENTE */
        public ArrayList GetListaAooPerUtente(string username)
        {
            ArrayList aooList = new ArrayList();

            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_AMM_LIST_ESPI");
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (dataSet != null)
                {
                    foreach (DataRow row in dataSet.Tables[0].Rows)
                    {
                        DocsPaVO.utente.AssociazioneAooUtente amm = new DocsPaVO.utente.AssociazioneAooUtente();
                        amm.userId = 0;
                        amm.associationId = 0;
                        amm.aooId = Convert.ToInt32(row[0].ToString());
                        amm.userName = row[1].ToString();
                        amm.allowRestrictedAccess = row[2].Equals(1) ? true : false;
                        amm.aooName = row[3].ToString();
                        aooList.Add(amm);
                    }
                }
            }
        ...
        /* REPERIMENTO LISTA DEGLI UTENTI ABILITATI ALLA AOO */
        public ArrayList GetUsersForAoo(string aooId)
        {

            ArrayList utenti = new ArrayList();

            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_UTENTI_BY_AOO_ESPI");
            q.setParam("idAmm", aooId);
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (dataSet != null)
                {
                    ...
                    {
                        DocsPaVO.utente.AssociazioneAooUtente amm = new DocsPaVO.utente.AssociazioneAooUtente();
                        amm.userId = Convert.ToInt32(row[0].ToString());
                        amm.associationId = 0;
                        amm.aooId = Convert.ToInt32(row[1].ToString());
                        amm.userName = row[2].ToString();
                        amm.allowRestrictedAccess = row[3].Equals(1) ? true : false;
                        amm.aooName = row[4].ToString();
                        utenti.Add(amm);
                    }
                }
            }
        ...
        /* LISTA AOO */
        public ArrayList GetAllAooForEspi()
        {
            ArrayList utenti = new ArrayList();

            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ALL_AMMINISTRAZIONI");
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (dataSet != null)
                {
                    foreach (DataRow row in dataSet.Tables[0].Rows)
                    {
                        ...
                        amm.aooId = Convert.ToInt32(row[0].ToString());
                        amm.userName = "";
                        amm.allowRestrictedAccess = false;
                        amm.aooName = row[1].ToString();
                        utenti.Add(amm);
                    }
                }
            }
        ...
        /* LISTA AOO */
        public ArrayList GetAssociationUserAooFromGiada(string id, string username)
        {
            ArrayList utenti = new ArrayList();

            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_AOO_USER_ASSOCIATION");
            q.setParam("idAmm", id);
            q.setParam("userName", username);
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (dataSet != null)
                {
                    foreach (DataRow row in dataSet.Tables[0].Rows)
                    {
                        ...
                        amm.aooName = row[1].ToString();
                        amm.userName = row[2].ToString();
                        amm.allowRestrictedAccess = row[3].Equals("0") ? false : true;
                        utenti.Add(amm);
                    }
                }
            }
